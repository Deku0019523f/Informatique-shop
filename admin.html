...
<body>
  ...
  <script>
    ...
    function loadChats() {
      ...
      db.collection('Messagerie').orderBy('created', 'desc').onSnapshot(snapshot => {
        userList.innerHTML = '';
        snapshot.forEach(doc => {
          const data = doc.data();
    ...
    function openChat(userId, name) {
      ...
      db.collection('Messagerie').doc(userId).collection('messages')
        .orderBy('timestamp')
        .onSnapshot(snapshot => {
          chatView.innerHTML = `<p><strong>${name}</strong></p>`;
          snapshot.forEach(doc => {
    ...
    function sendAdminMsg(e) {
      ...
      db.collection("Messagerie").doc(currentChatId).collection("messages").add({
        text,
        from: "admin",
